                    ...
                    if (currentBuild.result == null || currentBuild.result == 'SUCCESS') {
                        withCredentials([
                            ...
                            bat """
                                ...
                                :: Проверка и инициализация Git в целевой папке (если это не было сделано ранее)
                                if not exist .git (
                                    git init
                                    git remote add origin https://%GIT_USER%:%GIT_TOKEN%@github.com/DianaParygina/1LR.git
                                )

                                :: Скачиваем самый свежий код из обновленной main и принудительно обновляем локальный репозиторий
                                git fetch
                                git checkout main
